import type { AnalysisResult, Alert } from '@/types'

interface MarkerOpts {
  /** category 轴时每个窗口对应的 y 值（用于把标记点放在柱顶） */
  yValues?: number[]
  gradeColor: (a: Alert) => string
  gradeLabel: (a: Alert) => string
}
 ...
 */
export function buildAlertMarkers(result: AnalysisResult, opts: MarkerOpts) {
  const alerts = result.alerts || []
  const yOf = (wi: number) => (opts.yValues ? opts.yValues[wi] ?? 0 : 0)
    ...
    tooltip: {
      formatter: (p: any) => {
        ...
        if (a.silenced) {
          ...
          if (a.suppressedCount) lines.push(`已折叠 ${a.suppressedCount} 条同类重复告警`)
        }
